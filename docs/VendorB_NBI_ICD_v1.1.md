...
Doc ID: NBI-ICD-IMS-0042 | Issue: 1.1 | Status: Released
...
| 1.1 | 2026-06-08 | `VS.CpuUtilRatio` renamed to `VS.CpuUsageRatio`; `VS.InviteFailure5xx` and `VS.RegisterSuccessRatio` added to the IMS_PM measurement set. No FM/CM change. |
...
(see [1] §5.2). No local-time form is emitted.
...
**0.0 – 1.0** scale, not as a percentage. This applies to every `VS.*Ratio` measurement
type — including those introduced in Issue 1.1 — and to CM values carrying `unit="ratio"`.
...
Measurement types added in Issue 1.1 are appended with the next free `p` index; consumers
shall resolve values by `measType` name, never by a hard-coded `p` index.
...
| `VS.CpuUsageRatio` | float | **ratio** | 0.0..1.0 | Mean CPU utilisation. **Issue 1.1: renamed from `VS.CpuUtilRatio`; semantics and scale unchanged.** |
...
| `VS.InviteFailure5xx` | int | count | 0..`VS.InviteFailure` | **Issue 1.1.** INVITE transactions terminated with a 5xx class response |
| `VS.RegisterSuccessRatio` | float | **ratio** | 0.0..1.0 | **Issue 1.1.** Registration success ratio, computed as `VS.RegisterSuccess / VS.RegisterAttempt` over the granularity period (cf. §3.5) |
...
<measCollecFile xmlns="http://www.3gpp.org/ftp/specs/archive/32_series/32.435#measCollec">
  ...
  <measData>
    ...
    <measInfo measInfoId="IMS_PM">
      <granularityPeriod duration="PT15M" endTime="2026-06-07T15:15:00Z"/>
      <measType p="1">VS.RegisterAttempt</measType>
      <measType p="2">VS.RegisterSuccess</measType>
      <measType p="12">VS.CpuUsageRatio</measType>
      <measType p="16">VS.InviteFailure5xx</measType>
      <measType p="17">VS.RegisterSuccessRatio</measType>
...
Unchanged in Issue 1.1. Alarm semantics follow [3] / [4]. One `alarm` element per notification.
...
Unchanged in Issue 1.1. One snapshot per NE per day. The `unit` attribute is normative —
consumers shall not assume a unit from the parameter name.
